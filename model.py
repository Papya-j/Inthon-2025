...
# ========================
# Transformer Seq2Seq (개선된 모델)
# ========================

class TransformerSeq2Seq(nn.Module):
    """
    Transformer 기반 Seq2Seq 모델 (성능 개선 버전)
    
    구조:
    - 인코더: Multi-layer Transformer Encoder
    - 디코더: Multi-layer Transformer Decoder
    - Positional Encoding: 위치 정보 명시적 학습
    - Multi-head Attention: 전체 시퀀스 참조
    ...
    def __init__(self, in_vocab: int, out_vocab: int, **kwargs):
        ...
        self.pos_encoder = nn.Embedding(512, d_model)  # 최대 길이 512
        ...
        # Scaling factor (논문 "Attention is All You Need")
    ...
    def _generate_square_subsequent_mask(self, sz: int, device: torch.device) -> torch.Tensor:
        """
        Auto-regressive mask 생성 (미래 토큰을 볼 수 없도록)
        
        Returns:
            [sz, sz] bool tensor (True = mask)
        """
        mask = torch.triu(torch.ones(sz, sz, device=device), diagonal=1).bool()
        return mask
    
    def forward(self, src: torch.Tensor, tgt_inp: torch.Tensor, src_pad_id: int) -> torch.Tensor:
        """
        순전파 (학습 시)
        
        Args:
            src: [B, S] 입력 시퀀스
            tgt_inp: [B, T] 타겟 입력 시퀀스 (teacher forcing)
            src_pad_id: 소스 패딩 ID
        ...
        src_pos = torch.arange(S, device=device).unsqueeze(0).expand(B, -1)
        src_emb = self.embed_in(src) * self.scale + self.pos_encoder(src_pos)
        
        # Padding mask (True = 패딩)
        ...
        # Decoder
        # 임베딩 + Positional encoding
        tgt_pos = torch.arange(T, device=device).unsqueeze(0).expand(B, -1)
        tgt_emb = self.embed_out(tgt_inp) * self.scale + self.pos_decoder(tgt_pos)
    ...
    def generate(self, src: torch.Tensor, max_len: int, bos_id: int, eos_id: int, src_pad_id: int) -> torch.Tensor:
        ...
        for step in range(max_len):
            ...
            # 임베딩 + Positional encoding
            tgt_pos = torch.arange(T, device=device).unsqueeze(0).expand(B, -1)
            tgt_emb = self.embed_out(y) * self.scale + self.pos_decoder(tgt_pos)
            ...
            # 마지막 타임스텝의 logits
            logits = self.out_proj(dec_out[:, -1, :])  # [B, V]
...
class Model(BaseModel):
    ...
    def __init__(self) -> None:
        ...
        if use_transformer:
            ...
            # TinySeq2Seq 모델 인스턴스 생성 (기존 GRU 모델)
